...
observation, reward, done, info = env.step(action)## GO TO gym doc to display the default reward values
print (reward)
...
while not done:
    action = env.action_space.sample()
    state, reward, done, info = env.step(action) # go to gym doc to find the right params
...
alpha = 0.1
gamma = 0.6
epsilon = 0.1 # parametre qui détermine si nous allons préviligié l'exploration ou l'utilisation des valeurs apprise, le mettre entre 0.05 et 0.2
...
for i in range(1, 1000):
    ...
    epochs, penalties, reward, = 0, 0, 0 # initialize the params to 0
    done = False
    ...
    while not done:
        ...
        next_max = np.max(q_table[next_state]) # /!\ trouver l'index pour acceder a l'action d'apres
        
        new_value = (1 - alpha) * old_value + alpha * (reward + gamma * next_max) #C'est la formule que nous avons vu précédemment
        q_table[state, action] =  new_value # updater la table par la nouvelle valeur

        if reward == -10:
            penalties +=1  ## update la valeur des penalités
...
episodes = 100 # choisir le nombre d'épisode
...
total_epochs, total_penalties = 0, 0
episodes = 10000 # choisir le nombre d'épisode
